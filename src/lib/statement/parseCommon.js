...
export function parseStmtDate(v) {
  if (v == null) return null;
  ...
  // DD-MMM-YYYY  e.g. 05-Apr-2024 / 05 Apr 24
...
export function classifyHeaders(headerCells) {
  ...
  headerCells.forEach((cell, i) => {
    ...
    if (/(txn|transaction|value)?\s*date/.test(h) && roles.date == null) roles.date = i;
    else if (/(narration|description|particular|remarks|details|transaction remarks)/.test(h)) roles.description = i;
    else if (/(withdrawal|debit|paid out|dr)\b/.test(h)) roles.debit = i;
    else if (/(deposit|credit|paid in|cr)\b/.test(h)) roles.credit = i;
    else if (/(balance|closing bal)/.test(h)) roles.balance = i;
    else if (/amount/.test(h) && roles.amount == null) roles.amount = i;
  });
  return roles;
}

export function rowsFromTable(rows, roles) {
  const out = [];
  for (const r of rows) {
    const date = parseStmtDate(r[roles.date]);
    if (!date) continue;
